...
class WalletFirstLivePositionsMonitor {
  ...
  ensureWalletList() {
    ...
    const sorted = (Array.isArray(rows) ? rows : [])
      .map((row) => ({
        ...
        rankTs: Math.max(
          Number((row && row.liveScannedAt) || 0),
          Number((row && row.updatedAt) || 0),
          Number((row && row.all && row.all.lastTrade) || 0),
          Number((row && row.d30 && row.d30.lastTrade) || 0),
          Number((row && row.d24 && row.d24.lastTrade) || 0)
        ),
      }))
    ...
    this.warmWalletList = shardFiltered
      .filter((row) => Number(row.rankTs || 0) >= warmCutoff)
  ...
  loadPersistedState() {
    ...
    events.forEach((event) => {
      ...
      if (wallet && at > 0) this.recentActiveWalletAt.set(wallet, at);
    });
  ...
  noteRecentWalletActivity(wallet, at = Date.now()) {
    ...
    this.recentActiveWalletAt.set(normalized, Number(at || Date.now()));
  }
  ...
  pickBatchWallets(availableSlots = 0) {
    ...
    if (openWallets.length > 0 && this.hotWalletsPerPass > 0) {
      this.pickRotatingWallets(
        openWallets,
        Math.min(target, this.hotWalletsPerPass, openWallets.length),
        "openWalletCursor",
        seen,
        picked
      );
  ...
  chooseClient(options = {}) {
    ...
    for (let i = 0; i < this.restClientEntries.length; i += 1) {
      ...
      if (requests >= 30 && successes === 0 && failures >= 30) continue;
      if (requests >= 60 && successRate < 0.03) continue;
      ...
      const healthPenalty =
        preferHealthy && requests >= 4
          ...
          : 0;
      const score = inflight * 100 + timeoutPenalty + rateLimitPenalty + healthPenalty;
      if (!best) {
  ...
  markClientResult(idx, error = null) {
    const state = this.clientStates[idx];
    if (!state) return;
    const now = Date.now();
    const maybeDisableUnhealthyClient = () => {
      ...
      if (requests >= 160 && failures > successes * 4 && successRate < 0.15) {
        state.disabledUntil = now + 3 * 60 * 1000;
        state.cooldownUntil = Math.max(Number(state.cooldownUntil || 0), state.disabledUntil);
      }
    };
    if (!error) {
    ...
    if (message.toLowerCase().includes("failed to connect") || message.toLowerCase().includes("proxy")) {
      state.consecutiveTimeout = Math.min(10, Number(state.consecutiveTimeout || 0) + 1);
      state.cooldownUntil = now + Math.min(30000, this.rateLimitBackoffBaseMs * 2);
      if (state.consecutiveTimeout >= 6) {
        state.disabledUntil = now + 60 * 1000;
        state.cooldownUntil = Math.max(Number(state.cooldownUntil || 0), state.disabledUntil);
      }
  ...
  async fetchWalletPositions(wallet) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      try {
        ...
        this.markClientResult(idx, null);
        return rows;
      } catch (error) {
        this.markClientResult(idx, error);
        lastError = error;
  ...
  getStatus() {
    ...
    return {
      ...
      estimatedHotLoopSeconds:
        this.openWalletSet.size > 0 && this.hotWalletsPerPass > 0
          ? Math.ceil(
              (this.openWalletSet.size / Math.max(1, this.hotWalletsPerPass)) *
                (Math.max(1000, Number(this.status.lastPassDurationMs || this.targetPassDurationMs)) / 1000)
            )
          : null,
      estimatedWarmLoopSeconds:
        this.warmWalletList.length > 0 && this.warmWalletsPerPass > 0
          ? Math.ceil(
              (this.warmWalletList.length / Math.max(1, this.warmWalletsPerPass)) *
                (Math.max(1000, Number(this.status.lastPassDurationMs || this.targetPassDurationMs)) / 1000)
            )
          : null,
      shardIndex: this.shardIndex,
