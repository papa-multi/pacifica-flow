...
# Rebuild an account-endpoint-healthy proxy subset for the detector/census lane.
#
# Usage:
#   scripts/rebuild_detector_proxy_pool.sh
#   scripts/rebuild_detector_proxy_pool.sh --source data/indexer/working_proxies.txt --out data/live_positions/detector_proxies.txt

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SOURCE_FILE="$ROOT_DIR/data/indexer/working_proxies.txt"
OUT_FILE="$ROOT_DIR/data/live_positions/detector_proxies.txt"
TMP_DIR="/tmp"
PARALLEL=32
TIMEOUT_SEC=8
PASSES=2
MAX_LATENCY_MS=3000
MIN_OK=200
TEST_ACCOUNT="${PACIFICA_PROXY_TEST_ACCOUNT:-11111111111111111111111111111111}"
URL="https://api.pacifica.fi/api/v1/account?account=${TEST_ACCOUNT}"
...
UNIQ_FILE="$TMP_DIR/pf_detector_proxy_uniq_$$.txt"
OK_FILE="$TMP_DIR/pf_detector_proxy_ok_$$.txt"
STRICT_FILE="$TMP_DIR/pf_detector_proxy_strict_$$.txt"
FALLBACK_FILE="$TMP_DIR/pf_detector_proxy_fallback_$$.txt"
PROBE_SCRIPT="$TMP_DIR/pf_probe_detector_proxy_$$.sh"
...
body="$(printf "%s" "$resp" | sed '/__CODE__:/d;/__TIME__:/d')"
if [[ "$code" == "200" || "$code" == "404" || "$code" == "429" ]]; then
  if printf "%s" "$body" | grep -Eq '"error"[[:space:]]*:[[:space:]]*"Account not found"|"code"[[:space:]]*:[[:space:]]*404|Rate limit exceeded|"success"[[:space:]]*:[[:space:]]*true'; then
    printf "%s\t%s\n" "$p" "${time_total:-0}"
  fi
fi
EOS
...
echo "detector-proxy-pool rebuilt: total=$TOTAL ok=$OK_COUNT passes=$PASSES max_latency_ms=$MAX_LATENCY_MS test_account=$TEST_ACCOUNT out=$OUT_FILE"
