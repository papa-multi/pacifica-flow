...
(function () {
  ...
  const STORAGE_KEY = "pacificaFlow.copyTrading.selectedLeaderId";
  const VALID_PAGES = new Set(["overview", "settings"]);
  ...
  const state = {
    ...
    activityData: null,
    balanceData: null,
    executionLogData: null,
    reconciliationData: null,
    selectedLeaderId: loadSelectedLeaderId(),
    lastUpdatedAt: null,
  };
  ...
  function getPageFromPath(pathname) {
    const clean = String(pathname || window.location.pathname || "").replace(/\/+$/, "");
    if (clean.endsWith("/settings")) return "settings";
    if (clean.endsWith("/overview")) return "overview";
    if (clean === "/copy-trading" || clean === "/copy-trading/" || clean === "") return "overview";
    return "overview";
  }

  function loadSelectedLeaderId() {
    try {
  ...
  function saveSelectedLeaderId(value) {
    state.selectedLeaderId = String(value || "").trim();
    try {
      if (state.selectedLeaderId) {
        localStorage.setItem(STORAGE_KEY, state.selectedLeaderId);
      } else {
  ...
  function setPage(nextPage) {
    const normalized = VALID_PAGES.has(nextPage) ? nextPage : "overview";
    if (state.page === normalized) return;
    state.page = normalized;
    const path = normalized === "settings" ? "/copy-trading/settings" : "/copy-trading/overview";
  ...
  function formatSignedUsd(value, digits) {
    const num = Number(value);
    if (!Number.isFinite(num)) return "-";
  ...
  function formatRelative(value) {
    const num = Number(value);
    if (!Number.isFinite(num) || !num) return "-";
    const delta = Math.max(0, Math.floor((Date.now() - num) / 1000));
  ...
  function getChecked(form, name) {
    const el = form.elements && form.elements[name];
    if (!el) return false;
    if (el.length && el[0] && typeof el[0].checked === "boolean") {
      return Boolean(el[0].checked);
    }
    return Boolean(el && el.checked);
  }

  function getNumberField(form, name) {
    const raw = getTextField(form, name);
    if (!raw) return null;
  ...
  function chip(text, tone) {
    const klass = tone ? `ct-chip ${tone}` : "ct-chip";
    return `<span class="${klass}">${escapeHtml(text)}</span>`;
  }

  function rowChip(text, tone) {
    return chip(text, tone);
  }

  function statusTone(stateValue) {
    const value = String(stateValue || "").toLowerCase();
    if (["done", "good", "active", "running", "ready", "healthy", "copied", "ok", "enabled"].includes(value)) return "is-green";
    if (["current", "preview", "watch", "warning", "preview mode"].includes(value)) return "is-amber";
    if (["blocked", "bad", "failed", "error", "paused", "disabled", "stopped", "missing"].includes(value)) return "is-red";
    if (["info", "neutral", "idle"].includes(value)) return "is-blue";
    return "is-muted";
  }
  ...
  function computeModel() {
    ...
    const execution = snapshot.execution && snapshot.execution.config ? snapshot.execution : { config: {} };
    const config = execution.config || {};
    const runtime = snapshot.runtime || {};
    const profile = snapshot.profile || {};
    const risk = snapshot.risk || { global: {}, leaders: {} };
    const leaders = Array.isArray(snapshot.leaders) ? snapshot.leaders.slice() : [];
    const positions = Array.isArray(state.positionsData && (state.positionsData.copiedPositions || state.positionsData.positions))
      ? (state.positionsData.copiedPositions || state.positionsData.positions)
      : [];
    const activity = Array.isArray(state.activityData && state.activityData.activity) ? state.activityData.activity : [];
    const alerts = Array.isArray(state.activityData && state.activityData.alerts) ? state.activityData.alerts : [];
    const balanceHistory = Array.isArray(state.balanceData && state.balanceData.history) ? state.balanceData.history : [];
    const balance = state.balanceData && state.balanceData.latestBalance ? state.balanceData.latestBalance : null;
    const executionEvents = Array.isArray(state.executionLogData && state.executionLogData.events) ? state.executionLogData.events : [];
    const reconciliation = state.reconciliationData || null;
    const accessSets = Array.isArray(snapshot.apiKeys) ? snapshot.apiKeys : [];
    const hasCopyAccess = Boolean(snapshot.pacificaReady || state.positionsData && state.positionsData.hasConnectedAccessSet);
    const authenticated = Boolean(auth && auth.authenticated);
    const twoFactor = Boolean(auth && (auth.mfaEnabled || snapshot.twoFactorEnabled));
    const leadersReady = leaders.length > 0;
    const copyEnabled = Boolean(config.enabled);
    const dryRun = Boolean(config.dryRun);
    const copyActive = Boolean(copyEnabled && runtime.state === "running");
    const selectedLeader = leaders.find((row) => String(row.id || row.wallet || "").trim() === state.selectedLeaderId) || null;
    const selectedLeaderRisk = selectedLeader
      ? ((risk.leaders && (risk.leaders[String(selectedLeader.wallet || "").toUpperCase()] || risk.leaders[String(selectedLeader.id || "").toUpperCase()])) || {})
      : null;
    ...
    const readiness = [
      {
        ...
        detail: authenticated ? (auth.displayName || auth.email || auth.twitterUsername || "Account ready") : "Sign in to continue.",
      },
      {
        key: "twoFactor",
        label: "2FA enabled",
        done: twoFactor,
        blocked: authenticated && !twoFactor,
        detail: twoFactor ? "Authenticator is enabled." : "Enable 2FA before live copy trading.",
      },
      {
        key: "pacifica",
        label: "Pacifica connected",
        done: hasCopyAccess,
        blocked: authenticated && twoFactor && !hasCopyAccess,
        detail: hasCopyAccess ? "Pacifica access is connected." : "Connect a Pacifica access set.",
      },
      {
        key: "leaders",
        label: "Leaders added",
        done: leadersReady,
        blocked: authenticated && twoFactor && hasCopyAccess && !leadersReady,
        detail: leadersReady ? `${leaders.length} leader${leaders.length === 1 ? "" : "s"} configured.` : "Add at least one leader.",
      },
      {
        key: "copy",
        label: "Copy trading active",
        done: copyActive,
        blocked: authenticated && twoFactor && hasCopyAccess && leadersReady && !copyActive,
        detail: copyActive ? (dryRun ? "Preview mode is active." : "Live copy trading is running.") : "Copy trading is off.",
      },
      {
        key: "mode",
        label: "Mode",
        done: Boolean(snapshot && snapshot.execution && snapshot.execution.config && snapshot.execution.config.enabled !== undefined),
        blocked: false,
        detail: !snapshot || !snapshot.execution || !snapshot.execution.config
          ? "Waiting for saved execution settings."
          : dryRun
            ? "Dry run is active — no live orders will be sent."
            : "Live mode is enabled.",
      },
    ];

    const missing = readiness.find((step) => step.blocked) || null;
    let nextAction = { label: "Open settings", action: "settings" };
    let nextTitle = "Review your setup";
    let nextText = "Open the settings workspace to confirm execution, leaders, and risk controls.";

    if (!authenticated) {
      nextTitle = "Sign in to continue";
      nextText = "You need to be signed in before copy trading can be configured.";
      nextAction = { label: "Sign in", href: "/" };
    } else if (!twoFactor) {
      nextTitle = "Enable 2FA";
      nextText = "Live copy trading requires Google Authenticator before it can be activated.";
      nextAction = { label: "Enable 2FA", href: "/settings/" };
    } else if (!hasCopyAccess) {
      nextTitle = "Connect Pacifica";
      nextText = "Add a Pacifica access set so the workspace can read balance, positions, and execution state.";
      nextAction = { label: "Manage Pacifica", href: "/settings/" };
    } else if (!leadersReady) {
      nextTitle = "Add a leader";
      nextText = "Copy trading starts by selecting one or more leader wallets.";
      nextAction = { label: "Add leader", href: "/copy-trading/settings" };
    } else if (!copyEnabled) {
      nextTitle = "Enable copy trading";
      nextText = "Everything is ready. Turn copy trading on when you want the worker to start tracking leaders.";
      nextAction = { label: "Enable copy trading", action: "enable" };
    } else if (dryRun) {
      nextTitle = "Preview mode is active";
      nextText = "Dry run is on, so no live orders will be sent. Switch to live in settings when you're ready.";
      nextAction = { label: "Open settings", href: "/copy-trading/settings" };
    } else {
      nextTitle = "Copy trading is live";
      nextText = "The worker is active and ready to react to new leader positions.";
      nextAction = { label: "Pause copy trading", action: "pause" };
    }
  ...
  function renderHeader(model) {
    const page = state.page === "settings"
      ? {
          title: "Copy Trading Settings",
          subtitle: "Leader-first workspace for execution, risk, and manual behavior. Only real account state is shown.",
        }
      : {
          title: "Overview",
          subtitle: "Operational dashboard for readiness, leaders, copied positions, and copy-trading activity.",
        };

    const actions = state.page === "overview"
      ? renderOverviewHeaderActions(model)
      : `
  ...
  function renderOverviewHeaderActions(model) {
  ...
  function renderBanner(model) {
    const title = model.copyActive
      ? (model.dryRun ? "Copy trading is enabled in preview mode" : "Copy trading is live")
      : (!model.authenticated
        ? "Sign in to start copy trading"
        : !model.twoFactor
          ? "Enable 2FA before live trading"
          : !model.hasCopyAccess
            ? "Connect Pacifica before enabling copy trading"
            : !model.leadersReady
              ? "Add a leader to continue"
              : "Copy trading is off");

    const note = !model.authenticated
      ? "You need an active account before the workspace can show live copy-trading state."
      : !model.twoFactor
        ? "Google Authenticator is required before copy trading can be turned on."
        : !model.hasCopyAccess
          ? "Add a Pacifica access set so the app can read balance, positions, and execution state."
          : !model.leadersReady
            ? "The system is ready, but it still needs at least one tracked leader."
            : model.copyActive
              ? (model.dryRun ? "Dry run is active. The worker will preview trades, but it will not submit live orders." : "The worker is active and ready to react to tracked leader changes.")
              : "Everything is ready. Turn copy trading on when you want live tracking to begin.";

    const modeText = model.copyActive
      ? (model.dryRun ? "Preview mode" : "Live mode")
      : "Inactive";
    ...
    return `
      <section class="ct-banner">
        ...
        <div class="ct-actions">
          ...
          ${model.nextAction && model.nextAction.action === "enable"
            ? `<button class="ct-btn ct-btn-primary" type="button" data-action="enable">${escapeHtml(model.nextAction.label)}</button>`
            : model.nextAction && model.nextAction.action === "pause"
              ? `<button class="ct-btn ct-btn-primary" type="button" data-action="pause">${escapeHtml(model.nextAction.label)}</button>`
              : model.nextAction && model.nextAction.action === "enable-live"
                ? `<button class="ct-btn ct-btn-primary" type="button" data-action="enable-live">${escapeHtml(model.nextAction.label)}</button>`
                : model.nextAction && model.nextAction.href
                  ? `<a class="ct-btn ct-btn-primary" href="${escapeHtml(model.nextAction.href)}">${escapeHtml(model.nextAction.label)}</a>`
                  : `<a class="ct-btn ct-btn-primary" href="/copy-trading/settings" data-nav="settings">${escapeHtml(model.nextAction.label || "Open settings")}</a>`}
        </div>
      </section>
    `;
  }
  ...
  function renderSteps(model) {
    const steps = model.readiness.map((step, index) => {
      const tone = stepTone(step);
      const label = tone === "done" ? "Done" : tone === "blocked" ? "Blocked" : "Next";
      return `
  ...
  function renderReadinessCard(model) {
    const next = model.missing || model.readiness.find((step) => !step.done) || null;
    const statusRows = model.readiness.map((step) => {
      const tone = stepTone(step);
      const value = step.done ? "Ready" : step.blocked ? "Missing" : "Pending";
      return `
    ...
    const nextAction = model.nextAction && model.nextAction.href
      ? `<a class="ct-btn ct-btn-primary" href="${escapeHtml(model.nextAction.href)}">${escapeHtml(model.nextAction.label)}</a>`
      : model.nextAction && model.nextAction.action
        ? `<button class="ct-btn ct-btn-primary" type="button" data-action="${escapeHtml(model.nextAction.action)}">${escapeHtml(model.nextAction.label)}</button>`
        : "";
  ...
  function renderLeadersCard(model) {
    ...
    if (!leaders.length) {
      return `
        <section class="ct-card">
          ...
          ${renderEmptyState(
            "No tracked leaders",
            "You can add the first leader from the settings workspace. The system will snapshot any existing open positions as baseline when tracking begins.",
            `<a class="ct-btn ct-btn-primary" href="/copy-trading/settings" data-nav="settings">Add leader</a>`
          )}
        </section>
      `;
    ...
    const rows = leaders.map((leader) => {
      const active = String(leader.id || leader.wallet || "") === state.selectedLeaderId;
      const tone = leader.paused ? "is-amber" : leader.syncState && /drift|error|warn/i.test(String(leader.syncState)) ? "is-red" : "is-green";
      return `
        <button class="ct-row-card ct-leader-select-card ${active ? "is-selected" : ""}" type="button" data-select-leader="${escapeHtml(leader.id || leader.wallet)}">
          ...
          <div class="ct-meta-row">
            ${chip(`${formatPct(leader.allocationPct ?? 0, 0)} allocation`, "is-blue")}
            ${chip(`${formatUsd(leader.currentCopiedExposureUsd ?? 0, 0)} copied`, "is-muted")}
            ${chip(`${formatSignedUsd(leader.pnlUsd ?? 0, 0)}`, leader.pnlUsd > 0 ? "is-green" : leader.pnlUsd < 0 ? "is-red" : "is-muted")}
            ${chip(`${leader.copiedPositionsCount ?? 0} positions`, "is-muted")}
          </div>
        </button>
      `;
    }).join("");
  ...
  function renderCopiedPositionsCard(model) {
    ...
    const rows = positions.map((position) => {
      const symbol = formatSymbol(position.symbol || position.token);
      const side = String(position.side || "").toLowerCase();
      const pnl = Number(position.pnlUsd ?? position.unrealizedPnlUsd ?? position.pnl ?? 0);
      const status = position.status || position.lifecycleState || "Tracking";
      return `
        <tr>
          <td class="ct-mono">${escapeHtml(symbol)}</td>
          <td>${escapeHtml(side || "-")}</td>
          <td>${escapeHtml(position.leaderName || position.leaderWallet || "-")}</td>
          <td>${escapeHtml(formatNumber(position.copiedSize ?? position.size ?? position.amount ?? 0, 4))}</td>
          <td>${escapeHtml(formatNumber(position.entry ?? position.entryPrice ?? 0, 4))}</td>
          <td>${escapeHtml(formatNumber(position.mark ?? position.markPrice ?? 0, 4))}</td>
  ...
  function renderActivityCard(model) {
    const activity = (model.activity || []).slice(0, 8);
    const alerts = model.alerts || [];
    const list = activity.length
      ? activity.map((row) => {
          const tone = row.status === "copied" || /copied|ok|saved|enabled/i.test(String(row.status || row.message || "")) ? "is-green"
            : row.status === "blocked" || /block|fail|stop/i.test(String(row.status || row.message || "")) ? "is-red"
            : "is-amber";
          return `
            <div class="ct-summary-row">
              <div class="ct-summary-row-head">
      ...
      : renderEmptyState("No activity yet", "Execution events, skips, and manual overrides will appear here once copy trading is active.");

    const alertChips = alerts.length
      ? alerts.map((row) => chip(row.message || "Alert", row.severity === "good" ? "is-green" : row.severity === "warn" ? "is-amber" : "is-red")).join("")
      : chip("No active alerts", "is-muted");
  ...
  function renderBalanceCard(model) {
      ...
      return `
        <section class="ct-card">
          ...
          ${renderEmptyState(
            ...
            model.hasCopyAccess ? "The backend did not return a balance snapshot for the selected Pacifica access set." : "A Pacifica access set is required before balance can be read.",
            model.hasCopyAccess ? "" : `<a class="ct-btn ct-btn-primary" href="/settings/" data-nav="pacifica">Manage access</a>`
          )}
        </section>
      `;
    }
    ...
    return `
      <section class="ct-card">
        ...
        <div class="ct-card-grid">
          <div class="ct-summary-row">
            <div class="ct-summary-row-head">
              <strong>${escapeHtml(formatUsd(balance.amount ?? balance.balance ?? balance.pendingBalance ?? 0, 0))}</strong>
  ...
  function renderSettingsSummaryCard(model) {
    const summary = model.snapshot || {};
    const modeLabel = summary.execution && summary.execution.config
      ? (summary.execution.config.enabled ? (summary.execution.config.dryRun ? "Dry run" : "Live") : "Disabled")
      : "Disabled";
    const copyMode = summary.profile ? summary.profile.copyMode : "proportional";
    return `
      <section class="ct-card">
        <div class="ct-card-head">
          <div>
  ...
  function renderExecutionCard(model) {
    const config = (model.execution && model.execution.config) || {};
    const enabled = Boolean(config.enabled);
    const dryRun = Boolean(config.dryRun);
    const accessSet = Array.isArray(model.accessSets) ? model.accessSets.find((row) => row && row.id === config.accessKeyId || row && row.wallet === config.accessKeyId) : null;
    return `
      <section class="ct-card">
        <div class="ct-card-head">
          <div>
  ...
  function renderAccessSetOptions(accessSets, selectedId) {
    const rows = Array.isArray(accessSets) ? accessSets : [];
    if (!rows.length) {
      return `<option value="">No Pacifica access set saved</option>`;
    }
    return rows
      .map((row) => {
        const value = row.id || row.accessKeyId || row.wallet || "";
        const selected = String(value) === String(selectedId || "");
        const label = `${shortWallet(row.accountWallet || row.wallet)} · ${shortWallet(row.agentWallet || row.agentWalletPreview || "")}`;
        return `<option value="${escapeHtml(value)}" ${selected ? "selected" : ""}>${escapeHtml(label)}</option>`;
      })
      .join("");
  }

  function renderCopyModeOptions(selected) {
    const rows = [
    ...
    return rows
      .map(([value, label]) => `<option value="${value}" ${String(selected || "proportional") === value ? "selected" : ""}>${escapeHtml(label)}</option>`)
      .join("");
  }

  function renderLeaderList(model) {
    const leaders = model.leaders;
    const rows = leaders.length
      ? leaders.map((leader) => {
          const selected = String(leader.id || leader.wallet || "") === state.selectedLeaderId;
          return `
      ...
      : renderEmptyState(
          "No leaders added",
          "Add a leader wallet to start baseline-aware copy trading. Only new positions opened after tracking starts will be copied.",
          ""
        );
  ...
  function renderSelectedLeaderPanel(model) {
    const leader = model.selectedLeader;
    if (!leader) {
      return `
        <section class="ct-card">
          <div class="ct-card-head">
          ...
          ${renderEmptyState(
            "No leader selected",
            "Pick a leader from the list on the left. The selected leader panel will open here with only that leader’s settings."
          )}
        </section>
      `;
    }

    const leaderRisk = model.selectedLeaderRisk || {};
    const selectedWallet = String(leader.wallet || "").toUpperCase();
    return `
      <section class="ct-card">
        <div class="ct-card-head">
          <div>
  ...
  function renderRiskCard(model) {
    const global = (model.risk && model.risk.global) || {};
    return `
      <section class="ct-card">
        <div class="ct-card-head">
          <div>
  ...
  function renderBehaviorCard(model) {
    const profile = model.profile || {};
    const execution = model.execution && model.execution.config ? model.execution.config : {};
    return `
      <section class="ct-card">
        <div class="ct-card-head">
          <div>
  ...
  function renderAdvancedCard(model) {
    const execution = model.execution && model.execution.config ? model.execution.config : {};
    return `
  ...
  function render() {
    ...
    document.body.classList.remove("ct-page-overview", "ct-page-settings");
    document.body.classList.add(state.page === "settings" ? "ct-page-settings" : "ct-page-overview");
    document.title = `PacificaFlow | Copy Trading - ${state.page === "settings" ? "Settings Workspace" : "Overview"}`;
    ...
    root.innerHTML = state.page === "settings" ? renderSettings(model) : renderOverview(model);
  }
  ...
  async function refreshData(options) {
    ...
    try {
      const auth = await apiFetch("/api/auth/me").catch((error) => ({
        ok: false,
        authenticated: false,
        mfaEnabled: false,
        copyTradingEligible: false,
        error: error && error.message ? error.message : "auth unavailable",
      }));
      state.auth = auth || { authenticated: false };

      const eligible = Boolean(state.auth && state.auth.authenticated && (state.auth.copyTradingEligible || state.auth.mfaEnabled));

      if (eligible) {
        const [snapshot, positionsData, activityData, balanceData, executionLogData, reconciliationData] = await Promise.all([
          apiFetch("/api/copy/settings").catch(() => null),
          apiFetch("/api/copy/positions").catch(() => null),
          apiFetch("/api/copy/activity").catch(() => null),
          apiFetch("/api/copy/balance").catch(() => null),
          apiFetch("/api/copy/execution-log").catch(() => null),
          apiFetch("/api/copy/reconciliation").catch(() => null),
        ]);
        state.snapshot = snapshot && snapshot.ok ? snapshot : null;
        state.positionsData = positionsData && positionsData.ok ? positionsData : null;
        state.activityData = activityData && activityData.ok ? activityData : null;
        state.balanceData = balanceData && balanceData.ok ? balanceData : null;
        state.executionLogData = executionLogData && executionLogData.ok ? executionLogData : null;
        state.reconciliationData = reconciliationData && reconciliationData.ok ? reconciliationData : null;
        state.error = null;

        const leaders = (state.snapshot && Array.isArray(state.snapshot.leaders) ? state.snapshot.leaders : []);
        if (state.selectedLeaderId && !leaders.some((row) => String(row.id || row.wallet || "") === state.selectedLeaderId)) {
        ...
        state.executionLogData = null;
        state.reconciliationData = null;
        state.error = null;
      }
    } catch (error) {
      state.error = error && error.message ? error.message : "Failed to load copy trading data";
    } finally {
      state.loading = false;
  ...
  function reconnectStream() {
    ...
    if (!state.auth || !state.auth.authenticated || !(state.auth.copyTradingEligible || state.auth.mfaEnabled)) return;
    try {
      source = new EventSource("/api/copy/stream");
      source.addEventListener("message", () => {
        if (!state.refreshing) refreshData({ silent: true });
      });
      source.addEventListener("copy-event", () => {
        if (!state.refreshing) refreshData({ silent: true });
      });
      source.addEventListener("leaders-changed", () => {
        if (!state.refreshing) refreshData({ silent: true });
      });
  ...
  async function runCopyAction(action) {
    const routes = {
      enable: "/api/copy/enable",
      pause: "/api/copy/pause",
      "stop-keep": "/api/copy/stop-keep",
      "stop-close": "/api/copy/stop-close-all",
      "enable-live": "/api/copy/settings",
    };
    if (action === "enable-live") {
      const model = computeModel();
      const payload = buildGlobalExecutionPayload(model);
      payload.enabled = true;
      payload.dryRun = false;
      await apiFetch(routes.enableLive || "/api/copy/settings", {
        method: "PATCH",
    ...
    if (!route) return;
    await apiFetch(route, { method: "POST", body: "{}" });
    await refreshData({ silent: true });
  }

  function buildGlobalExecutionPayload(model, form) {
    const execution = model.execution && model.execution.config ? model.execution.config : {};
    const profile = model.profile || {};
    const accessKeyId = form ? getTextField(form, "accessKeyId") : String(execution.accessKeyId || "");
    return {
      enabled: form ? getChecked(form, "enabled") : Boolean(execution.enabled),
      dryRun: form ? getChecked(form, "dryRun") : Boolean(execution.dryRun),
      accessKeyId,
      copyMode: form ? getTextField(form, "copyMode") : String(profile.copyMode || execution.copyMode || "proportional"),
      copyOnlyNewTrades: form ? getChecked(form, "copyOnlyNewTrades") : Boolean(profile.copyOnlyNewTrades),
      allowReopenAfterManualClose: form ? !getChecked(form, "allowReopenAfterManualClose") : Boolean(profile.allowReopenAfterManualClose),
      pauseNewOpensOnly: form ? getChecked(form, "pauseNewOpensOnly") : Boolean(profile.pauseNewOpensOnly),
      stopMode: form ? getTextField(form, "stopMode") || "keep" : String(profile.stopMode || "keep"),
      copyRatio: form ? getNumberField(form, "copyRatio") : Number(execution.copyRatio || 1),
      fixedNotionalUsd: form ? getNumberField(form, "fixedNotionalUsd") : Number(execution.fixedNotionalUsd || 0),
      minNotionalUsd: form ? getNumberField(form, "minNotionalUsd") : Number(execution.minNotionalUsd || 0),
      slippagePercent: form ? getNumberField(form, "slippagePercent") : Number(execution.slippagePercent || 0.5),
      leveragePolicy: form ? getTextField(form, "leveragePolicy") : String(execution.leveragePolicy || "cap"),
      retryPolicy: form ? getTextField(form, "retryPolicy") : String(execution.retryPolicy || "balanced"),
      syncBehavior: form ? getTextField(form, "syncBehavior") : String(execution.syncBehavior || "baseline_only"),
    };
  }

  async function saveGlobalExecution(form) {
    const model = computeModel();
    const payload = buildGlobalExecutionPayload(model, form);
    await apiFetch("/api/copy/settings", {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
    await refreshData({ silent: true });
  ...
  async function saveGlobalRisk(form) {
    const payload = {
      global: {
        totalCopiedExposureCapUsd: getNumberField(form, "totalCopiedExposureCapUsd"),
        totalDailyLossCapUsd: getNumberField(form, "totalDailyLossCapUsd"),
        leverageCap: getNumberField(form, "leverageCap"),
        maxOpenPositions: getNumberField(form, "maxOpenPositions"),
        maxAllocationPct: getNumberField(form, "maxAllocationPct"),
        autoPauseAfterFailures: getChecked(form, "autoPauseAfterFailures") ? 3 : 0,
        allowedSymbols: String(getTextField(form, "allowedSymbols") || "")
          .split(/[,\s]+/g)
          .map((item) => item.trim().toUpperCase())
          .filter(Boolean),
        blockedSymbols: String(getTextField(form, "blockedSymbols") || "")
          .split(/[,\s]+/g)
          .map((item) => item.trim().toUpperCase())
          .filter(Boolean),
      },
  ...
  async function saveSelectedLeader(form) {
    const model = computeModel();
    const leader = model.selectedLeader;
    if (!leader) return;
    const leaderId = String(leader.id || leader.wallet || "").trim();
    const walletKey = String(leader.wallet || "").trim().toUpperCase();
    const leaderPatch = {
      allocationUsd: getNumberField(form, "allocationUsd"),
      allocationPct: getNumberField(form, "allocationPct"),
      stopMode: getTextField(form, "stopMode") || "keep",
      paused: Boolean(leader.paused),
    };
    const pausedBtn = form.querySelector(`[data-leader-action="pause"][data-leader-id="${CSS.escape(leaderId)}"]`);
    if (pausedBtn) {
      leaderPatch.paused = false;
    }
    const riskPatch = {
      global: {},
      leaders: {
        [walletKey]: {
          maxExposurePct: getNumberField(form, "maxExposurePct"),
          maxDrawdownPct: getNumberField(form, "maxDrawdownPct"),
          dailyLossCapUsd: getNumberField(form, "dailyLossCapUsd"),
          leverageCap: getNumberField(form, "leverageCap"),
          maxOpenPositions: getNumberField(form, "maxOpenPositions"),
          allowedSymbols: String(getTextField(form, "allowedSymbols") || "")
            .split(/[,\s]+/g)
    ...
    await apiFetch(`/api/copy/leaders/${encodeURIComponent(leaderId)}`, {
      method: "PATCH",
      body: JSON.stringify(leaderPatch),
    });
    await apiFetch("/api/copy/risk", {
      method: "PATCH",
      body: JSON.stringify(riskPatch),
    });
    await refreshData({ silent: true });
  }

  async function addLeader(form) {
    const payload = {
      wallet: getTextField(form, "wallet"),
      label: getTextField(form, "label"),
    };
    const response = await apiFetch("/api/copy/leaders", {
      method: "POST",
      body: JSON.stringify(payload),
    });
    const created = response && response.leader ? response.leader : null;
    if (created && (created.id || created.wallet)) {
      saveSelectedLeaderId(created.id || created.wallet);
    }
    form.reset();
    await refreshData({ silent: true });
  }

  async function saveManualOverride(form) {
    const payload = {
      symbol: getTextField(form, "symbol"),
      side: getTextField(form, "side"),
      leaderWallet: getTextField(form, "leaderWallet"),
      reason: getTextField(form, "reason"),
      closeOnce: getChecked(form, "closeOnce"),
      protectFromReopen: getChecked(form, "protectFromReopen"),
    };
    await apiFetch("/api/copy/manual-override", {
      method: "POST",
      body: JSON.stringify(payload),
    });
    form.reset();
  ...
  async function leaderAction(action, leaderId) {
    const model = computeModel();
    const leader = model.leaders.find((row) => String(row.id || row.wallet || "") === String(leaderId || ""));
    if (!leader) return;
    const id = String(leader.id || leader.wallet || "");
    if (action === "remove") {
      if (!window.confirm(`Remove leader ${leader.label || shortWallet(leader.wallet)}?`)) return;
      await apiFetch(`/api/copy/leaders/${encodeURIComponent(id)}`, { method: "DELETE" });
      if (state.selectedLeaderId === id) saveSelectedLeaderId("");
      await refreshData({ silent: true });
      return;
    }
    if (action === "pause") {
      await apiFetch(`/api/copy/leaders/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify({
          paused: !leader.paused,
          label: leader.label || "",
          note: leader.note || "",
          allocationPct: leader.allocationPct,
          allocationUsd: leader.allocationUsd,
          stopMode: leader.stopMode || "keep",
        }),
      });
      await refreshData({ silent: true });
      return;
  ...
  function onClick(event) {
    ...
    if (action) {
      ...
      if (action === "enable" || action === "pause" || action === "stop-keep" || action === "stop-close" || action === "enable-live") {
        event.preventDefault();
        runCopyAction(action).catch((error) => {
          state.error = error && error.message ? error.message : "Copy trading action failed";
          render();
        });
        return;
      }
    }
    const leaderId = target.getAttribute("data-select-leader");
    if (leaderId) {
      event.preventDefault();
      saveSelectedLeaderId(leaderId);
      render();
      return;
    }
    const leaderActionType = target.getAttribute("data-leader-action");
    const leaderActionId = target.getAttribute("data-leader-id");
    if (leaderActionType && leaderActionId) {
      event.preventDefault();
      leaderAction(leaderActionType, leaderActionId).catch((error) => {
        state.error = error && error.message ? error.message : "Leader action failed";
        render();
      });
      return;
    }
    const posAction = target.getAttribute("data-pos-action");
    if (posAction) {
      event.preventDefault();
      const symbol = target.getAttribute("data-symbol") || "";
      const side = target.getAttribute("data-side") || "";
      const actionText = posAction === "close" ? "Close copied position" : "Protect copied position";
      if (posAction === "close" && !window.confirm(`Close copied position ${symbol} ${side}?`)) return;
      apiFetch("/api/copy/manual-override", {
        method: "POST",
        body: JSON.stringify({
          symbol,
          side,
          reason: actionText,
          closeOnce: posAction === "close",
          protectFromReopen: posAction === "protect",
        }),
      })
        .then(() => (posAction === "close" ? apiFetch("/api/copy/run", { method: "POST", body: "{}" }) : Promise.resolve()))
        .then(() => refreshData({ silent: true }))
        .catch((error) => {
          state.error = error && error.message ? error.message : "Position action failed";
          render();
        });
    }
  }
  ...
  function onSubmit(event) {
    ...
    const mode = submitter && submitter.getAttribute("data-submit-mode");
    const promise = formName === "global-execution"
      ? saveGlobalExecution(form)
      : formName === "global-risk"
        ? saveGlobalRisk(form)
        : formName === "save-selected-leader"
          ? saveSelectedLeader(form)
          : formName === "add-leader"
            ? addLeader(form)
            : formName === "manual-override"
              ? saveManualOverride(form)
              : Promise.resolve();

    promise.catch((error) => {
      state.error = error && error.message ? error.message : "Save failed";
      render();
    });
  }

  function renderOverviewLayout(model) {
    return `
      <div class="ct-frame">
        <div class="ct-frame-inner">
          <div class="ct-shell">
  ...
  function renderSettingsLayout(model) {
    return `
      <div class="ct-frame">
        <div class="ct-frame-inner">
          <div class="ct-shell">
  ...
  function renderBody() {
    ...
    const model = computeModel();
    return state.page === "settings" ? renderSettingsLayout(model) : renderOverviewLayout(model);
  }
  ...
  function render() {
    document.body.classList.add("ct-page");
    document.body.classList.toggle("ct-page-settings", state.page === "settings");
    document.body.classList.toggle("ct-page-overview", state.page === "overview");
    document.title = `PacificaFlow | Copy Trading - ${state.page === "settings" ? "Settings Workspace" : "Overview"}`;
    if (state.error) {
      root.innerHTML = renderError(state.error);
      return;
