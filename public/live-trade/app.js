...
function extractLiveActiveWallets(payload) {
  const primary = extractRows(payload === null || payload === void 0 ? void 0 : payload.liveActiveWallets);
  if (primary.length > 0) return primary;
  return [];
}
function extractWalletPerformanceSupport(payload) {
  var _payload$summary;
  return asObject(payload === null || payload === void 0 ? void 0 : (_payload$summary = payload.summary) === null || _payload$summary === void 0 ? void 0 : _payload$summary.walletPerformanceSupport);
}
function payloadPublicTradesTotal(payload) {
  var _payload$summary2;
  const summaryTotal = n(payload === null || payload === void 0 ? void 0 : (_payload$summary2 = payload.summary) === null || _payload$summary2 === void 0 ? void 0 : _payload$summary2.publicTradesTotal, NaN);
...
function payloadWalletsTotal(payload) {
  var _payload$summary3;
  const summaryTotal = n(payload === null || payload === void 0 ? void 0 : (_payload$summary3 = payload.summary) === null || _payload$summary3 === void 0 ? void 0 : _payload$summary3.indexedWalletsTotal, NaN);
  if (Number.isFinite(summaryTotal)) return summaryTotal;
...
function payloadPositionsTotal(payload) {
  var _payload$summary4, _payload$summary5;
  const summaryTotal = n(payload === null || payload === void 0 ? void 0 : (_payload$summary4 = payload.summary) === null || _payload$summary4 === void 0 ? void 0 : _payload$summary4.openPositionsTotal, NaN);
  if (Number.isFinite(summaryTotal)) return summaryTotal;
  const legacyTotal = n(payload === null || payload === void 0 ? void 0 : (_payload$summary5 = payload.summary) === null || _payload$summary5 === void 0 ? void 0 : _payload$summary5.publicTradesTotal, NaN);
  if (Number.isFinite(legacyTotal)) return legacyTotal;
...
function liveActiveSignature(payload) {
  var _payload$summary6, _payload$summary7;
  const rows = extractLiveActiveWallets(payload);
  const sample = rows.slice(0, 12).map(row => {
    const item = asObject(row);
    return [normalizeWallet(item.wallet || ""), n(item.openPositions, 0), n(item.positionUsd, 0), tsMs(item.lastActivityAt || item.lastPositionAt || item.liveScannedAt || 0)].join("~");
  }).join("|");
  const total = Number.isFinite(n(payload === null || payload === void 0 ? void 0 : (_payload$summary6 = payload.summary) === null || _payload$summary6 === void 0 ? void 0 : _payload$summary6.liveActiveWalletsTotal, NaN)) ? n(payload === null || payload === void 0 ? void 0 : (_payload$summary7 = payload.summary) === null || _payload$summary7 === void 0 ? void 0 : _payload$summary7.liveActiveWalletsTotal, rows.length) : rows.length;
...
function titleCase(value) {
  return String(value || "").trim().replaceAll("_", " ").replace(/\s+/g, " ").replace(/\b\w/g, char => char.toUpperCase());
}
...
function derivePositionState(status, freshness, ageMs) {
  ...
  if (normalizedStatus.includes("clos")) return {
    label: "closing",
    tone: "down"
  };
  if (normalizedStatus.includes("reduc")) return {
    label: "reduced",
    tone: "down"
  };
  ...
  if (n(ageMs, Infinity) <= 15 * 60 * 1000) return {
    label: "new",
    tone: "up"
  };
  ...
  if (freshness === "cooling") return {
    label: "watch",
    tone: "cool"
  };
  return {
    label: normalizedStatus || freshness || "watch",
    tone: "cool"
  };
}
function topSymbolSharePct(symbolVolumes, totalVolumeUsd) {
  const values = Object.values(asObject(symbolVolumes)).map(value => n(value, NaN)).filter(value => Number.isFinite(value) && value > 0);
...
function computeWalletScores(metrics) {
  ...
  const avgTradeUsd = n(metrics.totalTrades, 0) > 0 ? n(metrics.totalVolumeUsd, 0) / Math.max(1, n(metrics.totalTrades, 0)) : 0;
  const avgTradeFactor = Math.min(1, avgTradeUsd / 25000);
  const liveEventFactor = Math.min(1, n(metrics.recentEvents15m, 0) / 15);
  const positiveWindows = [metrics.pnl7Usd, metrics.pnl30Usd, metrics.totalPnlUsd].filter(value => Number.isFinite(n(value, NaN)) && n(value, 0) > 0).length;
  const winRate = clamp(metrics.winRate, 0, 100);
  const concentrationPct = clamp(metrics.concentrationPct, 0, 100);
  const convictionScore = clamp(avgTradeFactor * 35 + exposureFactor * 30 + Math.min(n(metrics.openPositions, 0), 12) * 2 + liveEventFactor * 11 + concentrationPct * 0.12, 0, 100);
  const consistencyScore = clamp(winRate * 0.58 + positiveWindows * 12 + (n(metrics.totalTrades, 0) > 20 ? 12 : n(metrics.totalTrades, 0) > 5 ? 6 : 0), 0, 100);
  const riskScore = clamp(concentrationPct * 0.55 + exposureFactor * 25 + (metrics.freshness === "stale" ? 10 : 0) + (n(metrics.openPositions, 0) <= 1 && exposureFactor > 0.2 ? 12 : 0), 0, 100);
  const liveActiveScore = clamp(liveEventFactor * 45 + Math.min(n(metrics.openPositions, 0), 20) * 1.5 + exposureFactor * 25 + (metrics.freshness === "fresh" ? 20 : metrics.freshness === "cooling" ? 12 : metrics.freshness === "stale" ? 0 : 6), 0, 100);
  return {
    convictionScore,
    consistencyScore,
...
function deriveWalletBehaviorTags(metrics) {
  ...
  if (n(metrics.avgHoldingMs, 0) > 0 && n(metrics.avgHoldingMs, 0) <= 2 * 60 * 60 * 1000 && n(metrics.totalTrades, 0) >= 15) {
...
function buildWalletRankingGroups(rows) {
  ...
  const mostActive = list.filter(row => n(row.recentEvents15m, 0) > 0 || n(row.recentEvents1h, 0) > 0 || n(row.openPositions, 0) > 0).sort((a, b) => n(b.recentEvents15m, 0) - n(a.recentEvents15m, 0) || n(b.recentEvents1h, 0) - n(a.recentEvents1h, 0) || n(b.liveActiveScore, 0) - n(a.liveActiveScore, 0) || tsMs(b.lastActivity) - tsMs(a.lastActivity));
  const highestVolume = list.filter(row => n(row.volume7Usd, 0) > 0 || n(row.volume30Usd, 0) > 0 || n(row.totalVolumeUsd, 0) > 0 || n(row.exposureUsd, 0) > 0).sort((a, b) => Math.max(n(b.volume7Usd, 0), n(b.volume30Usd, 0), n(b.totalVolumeUsd, 0)) - Math.max(n(a.volume7Usd, 0), n(a.volume30Usd, 0), n(a.totalVolumeUsd, 0)) || n(b.exposureUsd, 0) - n(a.exposureUsd, 0));
  const highestWinRate = list.map(row => ({
    row,
    profile: walletWinRateProfile(row)
  })).filter(entry => entry.profile.valid).sort((a, b) => n(b.profile.rate, 0) - n(a.profile.rate, 0) || n(b.profile.trades, 0) - n(a.profile.trades, 0) || n(b.row.totalPnlUsd, 0) - n(a.row.totalPnlUsd, 0)).map(entry => ({
    ...entry.row,
    winRateProfile: entry.profile
  }));
  const strongestPnl = list.filter(row => Number.isFinite(n(row.pnl30Usd, NaN)) || Number.isFinite(n(row.pnl7Usd, NaN)) || Number.isFinite(n(row.totalPnlUsd, NaN))).sort((a, b) => walletPerformanceFocus(b).value - walletPerformanceFocus(a).value || n(b.totalPnlUsd, 0) - n(a.totalPnlUsd, 0));
  const highestConviction = list.filter(row => n(row.convictionScore, 0) > 0 || n(row.exposureUsd, 0) > 0).sort((a, b) => n(b.convictionScore, 0) - n(a.convictionScore, 0) || n(b.exposureUsd, 0) - n(a.exposureUsd, 0) || n(b.concentrationPct, 0) - n(a.concentrationPct, 0));
  const mostConsistent = list.filter(row => n(row.totalTrades, 0) >= 20 && n(row.consistencyScore, 0) > 0).sort((a, b) => n(b.consistencyScore, 0) - n(a.consistencyScore, 0) || n(b.totalTrades, 0) - n(a.totalTrades, 0) || n(b.totalPnlUsd, 0) - n(a.totalPnlUsd, 0));
  return [buildGroup("active", "Most Active Wallets", "Highest recent live activity and position motion.", "cool", mostActive), buildGroup("volume", "Highest Volume Wallets", "Largest recent traded volume and exposure.", "cool", highestVolume), buildGroup("winrate", "Highest Win Rate Wallets", "Best hit rate where sample size is meaningful.", "up", highestWinRate), buildGroup("pnl", "Strongest PnL Wallets", "Best realized + live performance windows.", "up", strongestPnl), buildGroup("conviction", "Highest Conviction Wallets", "Concentrated exposure with stronger persistence.", "cool", highestConviction), buildGroup("consistency", "Most Consistent Wallets", "Repeatable quality over a meaningful sample.", "up", mostConsistent)].filter(group => group.count > 0);
}
function walletGroupMetric(groupKey, row) {
  ...
  if (groupKey === "volume") {
    const primaryValue = n(row.volume7Usd, 0) > 0 ? row.volume7Usd : n(row.volume30Usd, 0) > 0 ? row.volume30Usd : row.totalVolumeUsd;
...
function buildRows(payload, options = {}) {
  ...
  const liveActiveSourceRows = includePositions || includeWallets || includeLiveActive ? extractLiveActiveWallets(payload).map((row, idx) => {
    const item = asObject(row);
  ...
  const liveActiveByWallet = new Map(liveActiveSourceRows.filter(row => walletLooksResolved(row.wallet)).map(row => [normalizeWallet(row.wallet), row]));
  ...
  const positionRows = includePositions ? (() => {
    const positionRowsFromPrimary = positionsPrimary.map((row, idx) => {
      ...
      const margin = n(item.margin, n(item.marginUsd, n(item.margin_usd, n(rawSnapshot.margin, n(rawSnapshot.margin_usd, NaN)))));
      ...
      const liquidationPrice = n(item.liquidationPrice, n(item.liquidation_price, n(rawSnapshot.liquidation_price, NaN)));
      ...
      const ageMs = openedAt ? Math.max(0, Date.now() - openedAt) : Number.isFinite(lifecycleAgeMs) ? lifecycleAgeMs : NaN;
      ...
      const leverage = Number.isFinite(margin) && margin > 0 && Number.isFinite(positionUsd) ? positionUsd / margin : NaN;
      ...
      return {
        ...
        walletLiveRank: (liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.rank) || null,
        walletOpenPositions: (liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.openPositions) || 0,
        walletRecentEvents15m: (liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.recentEvents15m) || 0,
        walletRecentEvents1h: (liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.recentEvents1h) || 0,
        lifecycle,
  ...
  const walletRows = includeWallets ? walletPerformance.map((row, idx) => {
    var _ref2, _ref3, _ref4, _ref5, _all$realizedPnlUsd, _ref6, _ref7, _all$unrealizedPnlUsd, _ref8, _all$volumeUsd, _all$trades, _ref9, _all$feesPaidUsd, _ref10, _ref11, _all$winRatePct, _d24$pnlUsd, _d7$pnlUsd, _d30$pnlUsd, _ref12, _lifecycle$avgObserve;
    const item = asObject(row);
    const all = asObject(item.all || item.d24 || item.d30 || {});
    ...
    const liveWallet = walletIdentity.resolved ? liveActiveByWallet.get(walletIdentity.wallet) : null;
    const historicalLastActivity = tsMs(all.lastTrade || all.last_trade || item.lastTradeAt || item.lastUpdatedAt || item.updatedAt || item.timestamp || 0);
    const liveLastActivity = tsMs((liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.lastActivityAt) || (liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.lastPositionAt) || (liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.liveScannedAt) || 0);
    ...
    const pnl24Usd = n((_d24$pnlUsd = d24.pnlUsd) !== null && _d24$pnlUsd !== void 0 ? _d24$pnlUsd : d24.realizedPnlUsd, NaN);
    const pnl7Usd = n((_d7$pnlUsd = d7.pnlUsd) !== null && _d7$pnlUsd !== void 0 ? _d7$pnlUsd : d7.realizedPnlUsd, NaN);
    const pnl30Usd = n((_d30$pnlUsd = d30.pnlUsd) !== null && _d30$pnlUsd !== void 0 ? _d30$pnlUsd : d30.realizedPnlUsd, NaN);
    const topSymbolSharePctValue = topSymbolSharePct(all.symbolVolumes, totalVolumeUsd);
    const openPositions = n(liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.openPositions, 0);
    const exposureUsd = n(liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.positionUsd, 0);
    const recentEvents15m = n(liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.recentEvents15m, 0);
    const recentEvents1h = n(liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.recentEvents1h, 0);
    const lifecycle = asObject(item.lifecycle);
    const avgHoldingMs = n((_ref12 = (_lifecycle$avgObserve = lifecycle.avgObservedHoldingMs) !== null && _lifecycle$avgObserve !== void 0 ? _lifecycle$avgObserve : lifecycle.avgCompletedHoldingMs) !== null && _ref12 !== void 0 ? _ref12 : lifecycle.currentAvgOpenAgeMs, NaN);
    ...
    const totalPnlUsd = (walletIdentity.resolved && Number.isFinite(realizedPnlUsd) ? realizedPnlUsd : 0) + (walletIdentity.resolved && Number.isFinite(unrealizedPnlUsd) ? unrealizedPnlUsd : 0);
    ...
    return {
      ...
      liveActiveRank: (liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.rank) || null,
      openPositions,
      exposureUsd,
      recentEvents15m,
      recentEvents1h,
      activityScore: n(liveWallet === null || liveWallet === void 0 ? void 0 : liveWallet.activityScore, NaN),
      concentrationPct: topSymbolSharePctValue,
      avgHoldingMs,
      increaseCount,
      reduceCount,
      ...
      topSymbols: Object.entries(asObject(all.symbolVolumes)).map(([symbol, value]) => [String(symbol || "").trim(), n(value, NaN)]).filter(entry => entry[0] && Number.isFinite(entry[1])).sort((a, b) => b[1] - a[1]).slice(0, 5),
      behaviorTags,
      lifecycle,
      ...derivedScores,
      raw: item
    };
  }).sort((a, b) => n(b.liveActiveScore, 0) - n(a.liveActiveScore, 0) || n(b.totalPnlUsd, 0) - n(a.totalPnlUsd, 0)) : null;
  const liveActiveRows = includeLiveActive ? liveActiveSourceRows : null;
  return {
    positions: positionRows,
    wallets: walletRows,
    liveActive: liveActiveRows
  };
}
...
function renderActivityRail(level, tone) {
  ...
  return `<span class="lt-activity-rail">${Array.from({
    length: 5
  }, (_, idx) => {
    const on = idx < active;
...
function renderMiniBars(values) {
  ...
  return `<span class="lt-mini-bars">${numbers.map(value => {
...
async function fetchJsonWithTimeout(url, timeoutMs = SNAPSHOT_TIMEOUT_MS, externalSignal = null) {
  ...
  if (externalSignal && controller) {
    if (externalSignal.aborted) controller.abort();else externalSignal.addEventListener("abort", abortFromExternal, {
      once: true
    });
  }
...
function renderPositionsTable(rows) {
  ...
  const columns = [{
    ...
    key: "activity",
    label: "Opened / Updated",
    align: "left",
    sortKey: "activity"
  }];
  head.innerHTML = `<tr>${columns.map(col => `<th class="${col.align === "right" ? "right" : "left"}" data-col="${escapeHtml(col.key)}"><button class="lt-table-sort-btn${state.positionsSortKey === col.sortKey ? " active" : ""}" type="button" data-position-sort-key="${escapeHtml(col.sortKey)}" data-position-sort-label="${escapeHtml(col.label)}">${escapeHtml(col.label)} <span data-position-sort-indicator>${state.positionsSortKey === col.sortKey ? state.positionsSortDir === "asc" ? "↑" : "↓" : "↑↓"}</span></button></th>`).join("")}</tr>`;
  head.querySelectorAll("[data-position-sort-key]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      state.pagination.positions.page = 1;
      fetchSnapshot().then(payload => applyPayload(payload, "sort")).catch(() => {});
      connectSse();
    });
  });
  if (!rows.length) {
    body.innerHTML = '<tr><td colspan="9" class="lt-empty-state">No open positions match the current filters.</td></tr>';
    return;
  }
  body.innerHTML = rows.map(row => {
    ...
    const activityTone = row.freshness === "fresh" ? "fresh" : row.freshness === "stale" ? "stale" : "cooling";
  ...
  body.querySelectorAll("tr[data-select-type='positions'][data-select-key]").forEach(rowNode => {
    rowNode.addEventListener("click", event => {
      ...
      if (button) return;
      const row = rows.find(entry => entry.key === String(rowNode.getAttribute("data-select-key") || ""));
      state.selection = {
        type: "positions",
...
function renderWalletsTable(rows) {
  ...
  if (!rows.length) {
    body.innerHTML = '<tr><td colspan="6" class="lt-empty-state">No wallets match the current filters.</td></tr>';
    return;
  }
  body.innerHTML = rows.map(row => {
    ...
    const realizedAll = Number.isFinite(n(row.realizedPnlUsd, NaN)) ? fmtSignedUsd(row.realizedPnlUsd, 2) : "n/a";
...
async function openWalletDrawerForRow(row) {
  ...
  renderDetailDrawer();
  try {
    const payload = await fetchJsonWithTimeout(`${WALLET_DETAIL_ENDPOINT_PREFIX}${encodeURIComponent(wallet)}`, DRAWER_FETCH_TIMEOUT_MS, abortController ? abortController.signal : null);
...
function renderWalletPositionsList(positions, sourceRow) {
  ...
  return `<div class="lt-drawer-positions-wrap">
    <table class="lt-drawer-positions-table">
      ...
      <tbody>
        ${rows.map(row => {
    const selected = sourceRow && sourceRow.tradeRef && row.id && String(sourceRow.tradeRef) === String(row.id);
    const side = normalizeSide(row.side);
    const sideClass = side.includes("long") || side === "buy" ? "up" : side.includes("short") || side === "sell" ? "down" : "flat";
...
function renderDetailDrawer() {
  ...
  if (state.drawer.loading) {
    var _state$drawer$sourceR, _state$drawer$sourceR2;
    drawerTitle.textContent = ((_state$drawer$sourceR = state.drawer.sourceRow) === null || _state$drawer$sourceR === void 0 ? void 0 : _state$drawer$sourceR.walletTitle) || ((_state$drawer$sourceR2 = state.drawer.sourceRow) === null || _state$drawer$sourceR2 === void 0 ? void 0 : _state$drawer$sourceR2.walletLabel) || shortWallet(state.drawer.wallet);
  ...
  if (state.drawer.error) {
    var _state$drawer$sourceR3, _state$drawer$sourceR4;
    drawerTitle.textContent = ((_state$drawer$sourceR3 = state.drawer.sourceRow) === null || _state$drawer$sourceR3 === void 0 ? void 0 : _state$drawer$sourceR3.walletTitle) || ((_state$drawer$sourceR4 = state.drawer.sourceRow) === null || _state$drawer$sourceR4 === void 0 ? void 0 : _state$drawer$sourceR4.walletLabel) || shortWallet(state.drawer.wallet);
  ...
  drawerTitle.textContent = (sourceRow === null || sourceRow === void 0 ? void 0 : sourceRow.walletTitle) || (sourceRow === null || sourceRow === void 0 ? void 0 : sourceRow.walletLabel) || shortWallet(detail.wallet || state.drawer.wallet);
  drawerSubtitle.textContent = `${titleCase(summary.freshness || "unknown")} • ${summary.liveActiveRank ? `#${fmtInt(summary.liveActiveRank)} live` : "tracked wallet"}`;
  ...
  const behaviorTags = deriveWalletBehaviorTags({
    ...
    concentrationPct: ((_safeArray$ = safeArray(summary.concentration)[0]) === null || _safeArray$ === void 0 ? void 0 : _safeArray$.sharePct) || 0,
    ...
    avgHoldingMs: lifecycle.avgObservedHoldingMs || lifecycle.avgCompletedHoldingMs || lifecycle.currentAvgOpenAgeMs,
  ...
  drawerBody.innerHTML = `
    ...
    <section class="lt-drawer-section">
      ...
      <div class="lt-drawer-list">
        ${safeArray(summary.concentration).length ? safeArray(summary.concentration).map(entry => `<div class="lt-drawer-list-item"><span>${escapeHtml(entry.symbol)}</span><strong>${escapeHtml(`${fmtUsd(entry.volumeUsd, 2)} • ${fmtPct(entry.sharePct, 1)}`)}</strong></div>`).join("") : '<div class="lt-drawer-empty">No concentration data available.</div>'}
      </div>
    </section>
    <section class="lt-drawer-section">
      <h4>Behavior context</h4>
      <div class="lt-tag-group">
        ${behaviorTags.length ? behaviorTags.map(tag => badge("cool", tag)).join(" ") : '<span class="lt-drawer-empty">No recent behavior signals.</span>'}
      </div>
    </section>
  `;
}
...
function renderHealth(payload) {
  var _payload$sync, _payload$marketContex, _payload$summary8;
  ...
  const walletFirst = asObject(payload === null || payload === void 0 ? void 0 : (_payload$summary8 = payload.summary) === null || _payload$summary8 === void 0 ? void 0 : _payload$summary8.walletFirstLive);
  ...
  if (connNode) {
    var _payload$sync2, _payload$summary9, _payload$environment2, _payload$marketContex2;
...
function renderObservability(payload) {
  ...
  const obs = asObject(payload === null || payload === void 0 ? void 0 : (_payload$summary15 = payload.summary) === null || _payload$summary15 === void 0 ? void 0 : _payload$summary15.observability);
  ...
  if (hotValue) {
    ...
    hotValue.textContent = capacity > 0 ? `${fmtInt(active)} / ${fmtInt(capacity)}` : `${fmtInt(active)}`;
  }
  const hotMeta = el("lt-obs-hot-meta");
  if (hotMeta) {
    hotMeta.textContent = `${fmtPct(hot.utilizationPct, 0)} util • dropped ${fmtInt(hot.droppedPromotions)}`;
  }
  ...
  if (coverageMeta) {
    var _payload$summary16, _payload$summary16$wa;
    const tracked = n(payload === null || payload === void 0 ? void 0 : (_payload$summary16 = payload.summary) === null || _payload$summary16 === void 0 ? void 0 : (_payload$summary16$wa = _payload$summary16.walletFirstLive) === null || _payload$summary16$wa === void 0 ? void 0 : _payload$summary16$wa.walletsWithOpenPositions, NaN);
...
function renderWalletSupportNote(payload) {
  ...
  const availableLabels = available.filter(entry => ["recent_trade_count", "recent_volume", "realized_pnl", "unrealized_pnl", "open_positions", "exposure", "symbol_concentration", "live_active_score", "freshness_state", "wallet_holding_time", "position_change_history"].includes(String(entry.key || ""))).map(entry => String(entry.label || "").trim()).filter(Boolean);
  const missingLabels = unavailable.slice(0, 3).map(entry => String(entry.label || "").trim()).filter(Boolean);
  note.hidden = false;
  note.innerHTML = `
    ...
    ${missingLabels.length ? `<div class="lt-wallet-support-muted">Hidden until a real dataset exists: ${escapeHtml(missingLabels.join(" • "))}.</div>` : ""}
  `;
}
function renderWalletRankingGroups(rows) {
  ...
  const activeGroupKey = groups.some(group => group.key === state.activeWalletGroup) ? state.activeWalletGroup : groups.some(group => group.key === "pnl") ? "pnl" : ((_groups$ = groups[0]) === null || _groups$ === void 0 ? void 0 : _groups$.key) || "";
  state.activeWalletGroup = activeGroupKey;
  const activeGroup = groups.find(group => group.key === activeGroupKey) || null;
  if (totalNode) {
    totalNode.textContent = activeGroup ? `${fmtInt(activeGroup.count)} wallets` : `${fmtInt(rows.length)} ranked wallets`;
  }
  if (!groups.length) {
  ...
  list.innerHTML = groups.map(group => {
    ...
    return `<button class="lt-wallet-group-card${selected ? " is-active" : ""}" type="button" data-wallet-group="${escapeHtml(group.key)}" aria-pressed="${selected ? "true" : "false"}">
        ...
        <div class="lt-wallet-group-summary">
          ...
          <span class="lt-wallet-group-preview">${activeGroupKey === group.key && group.rankedRows[0] ? escapeHtml(walletGroupMetric(group.key, group.rankedRows[0]).primary) : escapeHtml(group.subtitle)}</span>
        </div>
      </button>`;
  }).join("");
...
function render(options = {}) {
  ...
  const walletsActive = safeArray(walletGroupState === null || walletGroupState === void 0 ? void 0 : walletGroupState.activeRows);
...
function wireEvents() {
  var _el2, _el3, _el4, _el5, _el6, _el7, _el8, _el9, _el10, _el11, _el12, _el13, _el14, _el15, _el16, _el17, _el18, _el19, _el20, _el21, _el22, _el23;
  ...
  (_el4 = el("lt-drawer-close")) === null || _el4 === void 0 ? void 0 : _el4.addEventListener("click", () => {
    state.selection = {
      type: "",
      key: ""
    };
  ...
  (_el5 = el("lt-search")) === null || _el5 === void 0 ? void 0 : _el5.addEventListener("input", event => {
    state.search = String(event.target.value || "");
    resetPagination();
    render();
  });
  (_el6 = el("lt-filter-wallet")) === null || _el6 === void 0 ? void 0 : _el6.addEventListener("input", event => {
    state.filters.wallet = String(event.target.value || "");
    resetPagination();
    render();
  });
  (_el7 = el("lt-filter-symbol")) === null || _el7 === void 0 ? void 0 : _el7.addEventListener("change", event => {
    state.filters.symbol = String(event.target.value || "");
    resetPagination();
    render();
  });
  (_el8 = el("lt-filter-side")) === null || _el8 === void 0 ? void 0 : _el8.addEventListener("change", event => {
    state.filters.side = String(event.target.value || "");
    resetPagination();
    render();
  });
  (_el9 = el("lt-filter-min-usd")) === null || _el9 === void 0 ? void 0 : _el9.addEventListener("input", () => {
    state.filters.minUsd = readNumber("lt-filter-min-usd");
    resetPagination();
    render();
  });
  (_el10 = el("lt-filter-max-usd")) === null || _el10 === void 0 ? void 0 : _el10.addEventListener("input", () => {
    state.filters.maxUsd = readNumber("lt-filter-max-usd");
    resetPagination();
    render();
  });
  (_el11 = el("lt-filter-min-pnl")) === null || _el11 === void 0 ? void 0 : _el11.addEventListener("input", () => {
  ...
  (_el12 = el("lt-filter-max-pnl")) === null || _el12 === void 0 ? void 0 : _el12.addEventListener("input", () => {
  ...
  (_el13 = el("lt-filter-freshness")) === null || _el13 === void 0 ? void 0 : _el13.addEventListener("change", event => {
    state.filters.freshness = String(event.target.value || "");
    resetPagination();
    render();
  });
  (_el14 = el("lt-filter-tracked")) === null || _el14 === void 0 ? void 0 : _el14.addEventListener("change", event => {
    state.filters.trackedOnly = Boolean(event.target.checked);
    resetPagination();
    render();
  });
  (_el15 = el("lt-positions-page-size")) === null || _el15 === void 0 ? void 0 : _el15.addEventListener("change", event => {
  ...
  (_el16 = el("lt-wallets-page-size")) === null || _el16 === void 0 ? void 0 : _el16.addEventListener("change", event => {
    state.pagination.wallets.pageSize = Math.max(10, Math.floor(n(event.target.value, 100)));
    state.pagination.wallets.page = 1;
    fetchSnapshot().then(payload => applyPayload(payload, "page")).catch(() => {});
    connectSse();
  });
  (_el17 = el("lt-positions-page-prev")) === null || _el17 === void 0 ? void 0 : _el17.addEventListener("click", () => {
    state.pagination.positions.page = Math.max(1, n(state.pagination.positions.page, 1) - 1);
    fetchSnapshot().then(payload => applyPayload(payload, "page")).catch(() => {});
    connectSse();
  });
  (_el18 = el("lt-positions-page-next")) === null || _el18 === void 0 ? void 0 : _el18.addEventListener("click", () => {
    state.pagination.positions.page = n(state.pagination.positions.page, 1) + 1;
    fetchSnapshot().then(payload => applyPayload(payload, "page")).catch(() => {});
    connectSse();
  });
  (_el19 = el("lt-wallets-page-prev")) === null || _el19 === void 0 ? void 0 : _el19.addEventListener("click", () => {
    state.pagination.wallets.page = Math.max(1, n(state.pagination.wallets.page, 1) - 1);
    fetchSnapshot().then(payload => applyPayload(payload, "page")).catch(() => {});
    connectSse();
  });
  (_el20 = el("lt-wallets-page-next")) === null || _el20 === void 0 ? void 0 : _el20.addEventListener("click", () => {
    state.pagination.wallets.page = n(state.pagination.wallets.page, 1) + 1;
    fetchSnapshot().then(payload => applyPayload(payload, "page")).catch(() => {});
    connectSse();
  });
  (_el21 = el("lt-reset-filters")) === null || _el21 === void 0 ? void 0 : _el21.addEventListener("click", () => {
    state.search = "";
    state.filters = {
      wallet: "",
      symbol: "",
  ...
  (_el22 = el("lt-refresh-btn")) === null || _el22 === void 0 ? void 0 : _el22.addEventListener("click", async () => {
    try {
      const payload = await fetchSnapshot();
  ...
  (_el23 = el("lt-pause-btn")) === null || _el23 === void 0 ? void 0 : _el23.addEventListener("click", () => {
